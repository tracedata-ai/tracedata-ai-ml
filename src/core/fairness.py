...
class FairnessAnalyzer:
    ...
    def analyze_bias(self, attribute_name):
        ...
        results = {
            ...
            "privileged_favorable_rate": metric.num_positives(privileged=True)
            / metric.num_instances(privileged=True),
            "unprivileged_favorable_rate": metric.num_positives(privileged=False)
    ...
    def update_all_drivers_fairness(self):
        ...
        driver_stats = df.groupby("driver_id").agg(
            {"smoothness_score": "mean", "is_old": "first", "is_expert": "first"}
        )
        ...
        for driver_id, row in driver_stats.iterrows():
            metadata = {
                ...
                "diff_from_age_cohort": round(
                    row["smoothness_score"] - age_group_avgs.get(row["is_old"], 0), 2
                ),
                "status": (
                    "Above Average"
                    if row["smoothness_score"] >= age_group_avgs.get(row["is_old"], 0)
                    else "Below Average"
                ),
            }
    ...
    def update_all_trips_fairness(self):
        ...
        # Stub: wire per-trip cohort comparison once trip_id is joined in fairness payloads.
        self.get_fairness_data_with_ids()
    ...
    def update_all_persistence(self):
        ...
        for driver_id, row in driver_stats.iterrows():
            metadata = {
                "age_cohort_avg": round(age_group_avgs[row["is_old"]], 2),
                "diff": round(row["smoothness_score"] - age_group_avgs[row["is_old"]], 2),
            }
            cursor.execute(
                "UPDATE drivers SET fairness_metadata_json = ? WHERE driver_id = ?",
                (json.dumps(metadata), int(driver_id)),
            )
        ...
        for _, row in df.iterrows():
            ...
            cursor.execute(
                "UPDATE trips SET fairness_metadata_json = ? WHERE trip_id = ?",
                (json.dumps(metadata), int(row["trip_id"])),
            )
